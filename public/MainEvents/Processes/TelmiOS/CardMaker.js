...
async function main(drive, sdLayout, imageProfile, diskNumberParam) {
  const mode = getTelmiSyncParams().deviceMode
  if (mode === 'r36s') {
    mainR36s(drive, sdLayout, imageProfile, diskNumberParam).catch((e) => process.stderr.write((e && e.toString()) || 'r36s-flash-failed'))
    return
  }
...
if (_params_.length === 0) {
  ...
  main(_params_[0], _params_[1] || 'mono', _params_[2] || 'v20', _params_[3] || '').catch((e) => process.stderr.write(e.toString()))
}
